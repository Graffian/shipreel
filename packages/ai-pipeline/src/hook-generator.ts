import type { HookGenerationInput } from './types'

export async function generateHook(input: HookGenerationInput): Promise<string> {
  const prompt = buildHookPrompt(input)
  const result = await queryHookLLM(prompt)
  return result
}

function buildHookPrompt(input: HookGenerationInput): string {
...
async function queryHookLLM(prompt: string): Promise<string> {
