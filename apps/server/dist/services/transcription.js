...
export async function transcribeAudio(audioPath, duration) {
    console.log(`[transcription] Processing: ${audioPath}`);
    try {
        console.log('[transcription] Running: whisper --model tiny --language en');
        const output = execSync(`whisper "${audioPath}" --output-format json --language en --model tiny 2>&1`, { stdio: 'pipe', maxBuffer: 50 * 1024 * 1024, timeout: 300_000 }).toString();
